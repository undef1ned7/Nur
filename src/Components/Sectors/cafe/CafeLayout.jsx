...
export default function CafeLayout() {
  ...
  const shouldAutoPrintNow = async () => {
    ...
    const now = Date.now();
    if (
      bridgeHealthRef.current.ok === true &&
      now - bridgeHealthRef.current.checkedAt < 30_000
    )
      return true;
    if (
      bridgeHealthRef.current.ok === false &&
      now - bridgeHealthRef.current.checkedAt < 10_000
    )
      return false;
  ...
  const orderItemsSnapshotKey = useCallback(
    (orderId) => `cafe_order_items_snapshot_${orderId}`,
    [],
  );

  const readOrderItemsSnapshot = useCallback(
    (orderId) => {
      try {
  ...
  const writeOrderItemsSnapshot = useCallback(
    (orderId, snapshot) => {
      try {
        localStorage.setItem(
          orderItemsSnapshotKey(orderId),
  ...
  const resolveMenuKitchenId = async (menuItemId) => {
    ...
    if (!mid) return null;
    if (menuKitchenCacheRef.current.has(mid))
      return menuKitchenCacheRef.current.get(mid);
    try {
  ...
  const printKitchenDiffTicketsForOrder = useCallback(
    async (orderId) => {
      const oid = String(orderId || "");
      if (!oid) return;
