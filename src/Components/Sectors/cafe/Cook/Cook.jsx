...
const extractPortionsFromTask = (t) =>
  Math.max(
    1,
    Number(firstDefined(t?.quantity, t?.qty, t?.count, t?.portions, t?.amount, 1)) || 1
  );
...
const toUserMessage = (err) => {
  ...
  if (lower.includes("ингредиенты пустые") || lower.includes("ingredients") || lower.includes("menu-items") || lower.includes("menu items")) {
    return "Нельзя отметить «Готово»: у блюда не настроены ингредиенты. Обратитесь к администратору.";
  }
...
const Cook = () => {
  ...
  const groups = useMemo(() => {
    ...
    for (const t of base) {
      const orderId = pickOrderId(t);
      const minute = toMinuteKey(t?.created_at);
      const fallbackKey = `tbl:${String(t?.table_number ?? "")}|w:${String(t?.waiter_label ?? "")}|g:${String(t?.guest ?? "")}|c:${minute}`;
  ...
  const handleReadyOne = useCallback(
    ...
    [dispatch, refetch, showNotice, buildNeedForTask, applyWarehouseDecreaseSafe, applyWarehouseIncreaseSafe]
  );
