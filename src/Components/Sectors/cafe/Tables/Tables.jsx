...
const Tables = () => {
  ...
  // ✅ ПО УМОЛЧАНИЮ: “ЗАЛ” (как на 1-ом фото)
  const [tablesView, setTablesView] = useState("hall"); // manage | hall
  ...
  const fetchOrdersActive = useCallback(async () => {
    ...
    const active = (Array.isArray(all) ? all : []).filter((ord) => {
  ...
  // активные заказы по столам (ключи нормализуем в String)
  const activeByTable = useMemo(() => {
    const m = new Map();
  ...
  // ✅ сортировка столов по номеру + поиск
  const filteredTables = useMemo(() => {
    const q = query.trim().toLowerCase();
  ...
  const renderHall = () => (
    <div className="tables__hallGrid tables__scroll">
      {filteredTables.map((t) => {
        ...
        // собрать блюдо -> qty (без дублей, но x2 и т.п.)
        const agg = new Map();
        ...
        const isBusy = !!group; // занято, если есть активный заказ (даже если items ещё не догрузились)
        const date = formatHallDate(latest?.created_at || latest?.created || latest?.date);
