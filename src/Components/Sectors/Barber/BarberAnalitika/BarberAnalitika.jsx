...
/* ВАЖНО: для записей сначала берём start_at */
const tsOf = (x) => {
  const v =
    x?.start_at || // ← приоритет для записей барбершопа
    x?.sold_at ||
    x?.date ||
    x?.datetime ||
    x?.accepted_at ||
...
const BarberAnalitika = () => {
    ...
    try {
      ...
      const take = (resp, mode) => {
        (asArray(resp?.data) || []).forEach((r) => {
          ...
          if (!tId) return;
          map[tId] = map[tId] || {};
          if (mode === "lesson") map[tId].perRecord = Number(r.rate ?? 0) || 0;
          else map[tId].perMonth = Number(r.rate ?? 0) || 0;
        });
      };
  ...
  const filteredApps = useMemo(() => {
    return appointments.filter((a) => {
      const t = tsOf(a); // теперь берётся start_at
  ...
  return (
    <div className="barber-analitika">
      ...
      {/* Сетка каталога (без брендов и категорий) */}
      <section className="ba-grid">
        {renderCard({
          icon: <Users2 size={16} />,
          title: "Поставщики",
