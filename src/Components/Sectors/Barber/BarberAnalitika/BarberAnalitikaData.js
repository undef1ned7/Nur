...
//   /* ===== ставки мастеров (lesson + percent/month) ===== */
...
//             params: { period: periodLabel, mode, page_size: 1000 },
//           });
//         } catch {
//           return null;
//         }
...
//       let resMonth = null;
//       if (!resPercent) resMonth = await safeGet("month");
...
//           map[tId].percent = val;
//           if (mode === "month") map[tId].perMonth = val;
//         });
//       };
...
//     const income = cashRows.reduce(
//       (a, r) => a + toNum(r.income),
//       0
//     );
//     const expense = cashRows.reduce(
//       (a, r) => a + toNum(r.expense),
//       0
//     );
...
//       const addItem = (it) => {
//         const disp = clean(
//           it.product_name || it.object_name || it.name,
//           "—"
//         );
//         const k = keyOf(disp, "—");
//         names.set(k, disp);
...
//             ).unwrap();
//             (Array.isArray(d1?.items) ? d1.items : []).forEach(
//               addItem
//             );
//           } catch (e) {
//             console.error(e);
...
//             ).unwrap();
//             (Array.isArray(d2?.items) ? d2.items : []).forEach(
//               addItem
//             );
//           } catch (e) {
//             console.error(e);
...
//   /* ===== выплаты и единый приход/расход ===== */
...
//         const fixed = percent ? 0 : perMonth;
...
//         for (const cf of periodFlows) {
//           const id = String(cf.cashbox ?? cf.cashbox_id ?? "");
//           const amount = toNum(cf.amount ?? cf.value ?? cf.sum ?? 0);
//           const dir = amount >= 0 ? "income" : "expense";
//           const amtAbs = Math.abs(amount);
//           const prev = m.get(id) || { income: 0, expense: 0, ops: 0 };
//           m.set(id, {
//             income: prev.income + (dir === "income" ? amtAbs : 0),
//             expense: prev.expense + (dir === "expense" ? amtAbs : 0),
//             ops: prev.ops + 1,
//           });
//         }
...
//   // Продажи товаров – всего шт и выручка
...
//   // Клиенты продаж – активные клиенты и выручка
...
//   // Записи по дням недели (для бар-чарта)
//   const weekChart = useMemo(() => {
//     const counts = [0, 0, 0, 0, 0, 0, 0]; // Пн-Вс
//     filteredApps.forEach((a) => {
//       const t = tsOf(a);
//       if (!t) return;
//       const d = new Date(t);
//       const idx = (d.getDay() + 6) % 7; // getDay: 0=Вс, сдвиг к 0=Пн
//       counts[idx] += 1;
//     });
//     return counts;
//   }, [filteredApps]);

//   // Динамика доходов/расходов по дням месяца, на основе кэшфлоу
...
//       const amount = toNum(cf.amount ?? cf.value ?? cf.sum ?? 0);
//       if (amount >= 0) income[idx] += amount;
//       else expense[idx] += Math.abs(amount);
//     });
...
export const useBarberAnalitikaData = ({ year, monthIdx }) => {
    ...
    (async () => {
      ...
      try {
        ...
        for (const cf of periodFlows) {
          ...
          m.set(id, {
            income:
              prev.income + (direction === "income" ? amtAbs : 0),
            expense:
              prev.expense + (direction === "expense" ? amtAbs : 0),
            ops: prev.ops + 1,
          });
        }
  ...
  // Приход = завершённые записи + приход по кассам
  // Расход = расход по кассам
  const dayLineChart = useMemo(() => {
    const daysInMonth = new Date(year, monthIdx + 1, 0).getDate();
    const income = Array(daysInMonth).fill(0);
    const expense = Array(daysInMonth).fill(0);
    ...
    // 2) Кассовые операции (construction cashflow)
    cashflowsPeriod.forEach((cf) => {
      const t = tsOf(cf);
      if (!t) return;
      const d = new Date(t);
