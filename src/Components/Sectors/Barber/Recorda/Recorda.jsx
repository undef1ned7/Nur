...
/* парсинг процента скидки из текстового поля (можно "10", "10%", "-5" и т.п.) */
...
/* вычисление финальной цены по базе и скидке в % */
const calcFinalPrice = (base, discountPercent) => {
...
const ComboBox = ({
  ...
  return (
    <div
      className={`barberrecorda__combo ${
        open ? "is-open" : ""
      }`}
      ref={wrapRef}
    >
  ...
  return (
    ...
    >
      <select
        ...
        onChange={(e) =>
          setHM(parseInt(e.target.value, 10), m || 0)
        }
      >
        {hours.map((H) => (
...
const ServicesPicker = ({
  ...
  const available = items.filter(
    (i) => !already.has(String(i.id))
  );
  const onPick = (id) => {
    const sid = String(id);
    if (!already.has(sid)) onChange([...selectedIds, sid]);
  };
  const remove = (sid) =>
    onChange(
      selectedIds.filter((id) => String(id) !== String(sid))
    );
      ...
      {selectedIds.length > 0 && (
        <>
          <div className="barberrecorda__svcSummary">
            ...
            <span>·</span>
            <span>
              Время: <b>{summary.totalMinutes}</b> мин
            </span>
            <span>·</span>
            <span>
              Сумма:{" "}
              <b>{fmtMoney(summary.totalPrice)}</b>
            </span>
          </div>
          ...
          <div className="barberrecorda__svcList">
            {selectedIds.map((id) => {
              ...
              return (
                <span
                  key={id}
                  className="barberrecorda__svcChip"
                  title={name}
                >
                  <span className="barberrecorda__svcName">
                    {name}
                  </span>
                  <span className="barberrecorda__svcMeta">
                    {mm ? `⏱ ${mm} мин` : "—"}
                  ...
                  <button
                    type="button"
                    className="barberrecorda__svcDel"
...
const Recorda = () => {
    ...
    try {
      ...
      const svcs = asArray(sv.data)
        ...
        .map((s) => ({
          ...
          price:
            s.price == null ? null : Number(s.price),
          time: s.time || "",
  ...
  const totalSlots =
    (timeBounds.endH - timeBounds.startH) *
    (60 / SLOT_MIN);
  ...
  const busySlots = useMemo(() => {
    ...
    for (let i = 0; i < totalSlots; i++) {
      const slotStart =
        timeBounds.startH * 60 + i * SLOT_MIN;
      const slotEnd = slotStart + SLOT_MIN;
      const busy = dayRecords.some((r) => {
  ...
  const serviceNamesFromRecord = (r) => {
    if (
      Array.isArray(r.services_names) &&
      r.services_names.length
    )
  ...
  const layoutForBarber = (list) => {
    const items = list.map((r) => {
      ...
      const heightByTime = durMin * PX_PER_MIN;
      const heightByText = estimateContentMin(
        svc,
        cl,
        ph
      );
    ...
    items.forEach((it) => {
      ...
      if (lane === lanes.length) lanes.push(it.tEnd);
      else lanes[lane] = it.tEnd;
      it.lane = lane;
    ...
    items.forEach((it) => {
      const widthPct =
        (100 - (it.lanes - 1) * GAP) / it.lanes;
      const { bg, border, shadow } = colorByStatus(
        it.r.status
      );
      it.style = {
        top: `${it.top}px`,
    ...
    () =>
      services
        ...
        .map((s) => ({
          id: String(s.id),
          label: s.name,
          search: `${s.name} ${s.time || ""} ${
            s.price || ""
          }`,
          price: Number.isFinite(s.price)
            ? Number(s.price)
            : null,
          minutes: s.minutes || 0,
        })),
  ...
  /* перерасчёт цены по услугам и скидке */
  useEffect(() => {
    if (!modalOpen) return;
    if (!basePrice) {
      setPriceInput("");
  ...
  }, [basePrice, discountInput, modalOpen]);
  ...
  const busyBarbersOnInterval = useMemo(() => {
    ...
    const t1 = ts(selectedStartISO),
      t2 = ts(selectedEndISO);
    appointments.forEach((a) => {
      if (toDate(a.start_at) !== startDate) return;
      if (!BLOCKING.has(a.status)) return;
      if (
        current?.id &&
        String(current.id) === String(a.id)
      )
        return;
      if (
        overlaps(
  ...
  const openModal = (rec = null) => {
    ...
    if (rec) {
      ...
      setStartDate(toDate(rec.start_at));
      setStartTime(
        clampToRange(toTime(rec.start_at))
      );
      setEndTime(clampToRange(toTime(rec.end_at)));
      setAutoEnd(false);
      setSelBarber(String(rec.barber || ""));
      ...
      setDiscountInput(
        rec.discount !== null &&
        rec.discount !== undefined
          ? String(rec.discount)
          : ""
      );
      // priceInput пересчитается через useEffect
    } else {
      setSelClient("");
      setStartDate(fltDate || todayStr());
      setSelServices([]);
      const now = new Date();
      const tNow = `${pad(now.getHours())}:${pad(
        now.getMinutes()
      )}`;
      const st = inRange(tNow)
        ? clampToRange(tNow)
        : `${pad(OPEN_HOUR)}:00`;
      setStartTime(st);
      setAutoEnd(true);
  ...
  const validate = () => {
    ...
    if (!errs.startTime && !errs.endTime) {
      if (!(inRange(startTime) && inRange(endTime))) {
        ...
        errs.endTime = true;
        alerts.push(
          "Время должно быть в пределах 09:00–21:00."
        );
      } else if (eM <= sM) {
        errs.endTime = true;
    ...
    const conflictsMaster = appointments.filter(
      (a) => {
        if (
          String(a.barber) !== String(selBarber)
        )
    ...
    const conflictsClient = appointments.filter(
      (a) => {
        if (String(a.client) !== String(selClient))
          return false;
        if (!BLOCKING.has(a.status)) return false;
        if (
          current?.id &&
    ...
    if (conflictsClient.length) {
      errs.startTime = true;
      alerts.push(
        "У клиента уже есть запись в этот интервал."
      );
    }
...
const submit = async (e) => {
  ...
  // === вот отсюда начинается новый блок про скидку и цену ===
  const discountVal = parsePercent(discountInput);

  let finalPrice = null;
  const rawPrice = String(priceInput || "").trim();

  // 1) если в поле "Цена" что-то ввели — берём именно это
  if (rawPrice !== "") {
  ...
  // 2) если поле пустое/мусор — считаем по услугам + скидке
  if (finalPrice === null) {
  ...
  try {
    const payload = {
      ...
      company: localStorage.getItem("company"),
      discount:
        discountVal !== null && discountVal !== undefined
          ? discountVal
          : null,
      price:
  ...
  const closeMini = () => {
    if (!miniSaving) setMiniOpen(false);
  };
  const saveMini = async (e) => {
    e?.preventDefault?.();
    setMiniSaving(true);
                ...
                <section key={b.id} className="barberrecorda__calCol">
                  ...
                  >
                    {layout.length === 0 && !loading && (
                      <div className="barberrecorda__emptyInCol">Свободно</div>
                    )}
                    {layout.map((it) => {
                      const r = it.r;
                      const svc = serviceNamesFromRecord(r);
      ...
      <div className="barberrecorda__overlay" onClick={closeModal}>
        ...
        >
          ...
          <form
            ...
            noValidate
          >
            <div className="barberrecorda__grid barberrecorda__grid--split">
              ...
              <div className="barberrecorda__gridMain">
                ...
                >
                  <span className="barberrecorda__label">
                    Клиент{" "}
                    <b className="barberrecorda__req">*</b>
                  </span>
                  <div className="barberrecorda__fieldRow">
                    <ComboBox
                ...
                <label className="barberrecorda__field barberrecorda__field--full">
                  ...
                  <textarea
                    ...
                    placeholder="Заметка для сотрудника/клиента"
                  />
                </label>
              </div>

              {/* ПРАВАЯ КОЛОНКА — услуги, скидка, цена */}
              <aside className="barberrecorda__gridSide">
                <div className="barberrecorda__sideCard">
                  {/* Услуги */}
                  <label
                    className={`barberrecorda__field barberrecorda__field--services ${
                  ...
                  <div className="barberrecorda__sideRow">
                    {/* Скидка */}
                    <label className="barberrecorda__field">
                      <span className="barberrecorda__label">
                        Скидка, %
...
<input
  ...
  className="barberrecorda__input"
  value={priceInput}
  onChange={(e) => setPriceInput(e.target.value)}    // можно руками менять
