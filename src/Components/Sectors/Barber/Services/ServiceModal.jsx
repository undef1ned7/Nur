...
//   // === ВАЖНОЕ ИЗМЕНЕНИЕ: показываем в селекте только активные категории,
//   // но если у услуги уже стоит неактивная категория — её тоже оставляем в списке.
//   const safeCategories = Array.isArray(categories) ? categories : [];
//   const currentCategoryId = currentService?.categoryId ?? "";
//   const categoriesForSelect = safeCategories.filter((c) => {
//     if (c.active) return true; // активные всегда показываем
//     // если редактируем услугу и у неё стоит неактивная категория — тоже показываем,
//     // чтобы значение не "пропало" из селекта
...
const ServiceModal = ({
  ...
  // показываем активные категории; если у услуги уже стоит неактивная — тоже показываем
  const safeCategories = Array.isArray(categories) ? categories : [];
  const currentCategoryId = currentService?.categoryId ?? "";
  const categoriesForSelect = safeCategories.filter((c) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ категория теперь НЕ обязательна
    ...
    try {
      ...
      const payload = {
        ...
        is_active: active,
        time: time || null,
        // ✅ если пусто — отправляем null (и это ок для бэка)
        ...
        <form className="barberservices__form" onSubmit={handleSubmit} noValidate>
          <div className="barberservices__grid">
            ...
            <label className="barberservices__field">
              ...
              <select
                name="category"
                // ✅ теперь может быть пусто
                defaultValue={currentService?.categoryId || ""}
                className="barberservices__input barberservices__input--select"
              >
                {/* ✅ больше НЕ disabled */}
