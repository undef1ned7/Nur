...
// const CashReports = () => {
//   const now = new Date();
//   const [tab, setTab] = useState("catalog"); // analytics | catalog | barber | payouts
...
//   const persistRates = async ({ perRecordPeriod, perMonthPeriods } = {}) => {
//     setRatesLoading(true);
//     setRatesError("");
//     try {
//       const tasks = [];
...
//         };
//         // ставка/запись на период
//         tasks.push(send("lesson", rec.perRecord, "id_lesson", perRecordPeriod || periodLabel));
//         // фикс/месяц только на выбранные периоды
//         const months = perMonthPeriods?.length ? perMonthPeriods : [periodLabel];
//         months.forEach((p) => tasks.push(send("month", rec.perMonth, "id_month", p)));
//       });
//       await Promise.allSettled(tasks);
//       await loadRates(); // подтянуть id после создания/обновления
//     } catch (e) {
//       console.error(e);
...
//   // При смене периода — чистим ставки и грузим только их (быстро)
//   useEffect(() => {
//     setRates({});
//     setRatesError("");
//     loadRates();
...
//   const totalClientsMarket = useMemo(() => clientsMarket.filter((c) => !isSupplierLike(c)).length, [clientsMarket]);
...
//   const top10Clients = useMemo(() => [...rankClientsVisits].sort(byDescCount).slice(0, 10), [rankClientsVisits]);
...
//       const done = filteredApps.filter((a) => a.barber && String(a.barber) === String(e.id) && a.status === "completed").length;
//       const r = rates[e.id] || {};
...
//         <button className="barbercashreports__pageBtn" onClick={onPrev} disabled={page === 1}>
//           Назад
//         </button>
...
//           })}
//         </ul>
//         <button className="barbercashreports__pageBtn" onClick={() => onNext(page + 1)} disabled={page === totalPages}>
//           Далее
//         </button>
//       </nav>
//     );
...
//             Каталог
//           </button>
//           <button className={`barbercashreports__tab ${tab === "barber" ? "is-active" : ""}`} onClick={() => setTab("barber")}>
//             Барбер аналитика
//           </button>
...
//               <span className="barbercashreports__subtitle">{loading ? "Загрузка…" : `Период: ${pad2(month)}.${year}`}</span>
//             </div>
...
//                     </div>
//                     {row.code === "completed" && <div className="barbercashreports__statusMoney">Сумма: {fmtMoney(row.sum)}</div>}
//                   </div>
//                 );
//               })}
...
//                 <button className="barbercashreports__iconBtn" title="Показать всё" onClick={() => setFullView("barbers")}>
//                   <FaListUl />
//                 </button>
//               </div>
...
//                 <button className="barbercashreports__iconBtn" title="Показать всё" onClick={() => setFullView("services")}>
//                   <FaListUl />
//                 </button>
//               </div>
...
//               <button className="barbercashreports__iconBtn" title="Показать полный список" onClick={() => setFullView("topClients")}>
//                 <FaListUl />
//               </button>
//             </div>
...
//                     {fullView === "barbers" ? "Мастера — записи" : fullView === "services" ? "Услуги — использование" : "Клиенты"}
//                   </h3>
//                   <button className="barbercashreports__iconBtn" onClick={() => setFullView(null)} aria-label="Закрыть">
//                     <FaTimes />
//                   </button>
//                 </div>
...
//                 <nav className="barbercashreports__pager" aria-label="Пагинация">
//                   <button className="barbercashreports__pageBtn" onClick={() => setModalPage((p) => Math.max(1, p - 1))} disabled={modalPage === 1}>
//                     Назад
//                   </button>
//                   <button className="barbercashreports__pageBtn" onClick={() => setModalPage((p) => p + 1)}>
//                     Далее
//                   </button>
//                 </nav>

//                 <div className="barbercashreports__formActions">
//                   <button className="barbercashreports__btn barbercashreports__btn--secondary" onClick={() => setFullView(null)}>
//                     Закрыть
//                   </button>
//                 </div>
//               </div>
...
const CashReports = () => {
  ...
  // показываем только две вкладки; по умолчанию — "barber"
  const [tab, setTab] = useState("barber"); // barber | payouts

  // данные барбершопа для компонента BarberAnalitika и для выплат
  const [appointments, setAppointments] = useState([]);
  const [employees, setEmployees] = useState([]);
  const [services, setServices] = useState([]);
  ...
  /* ===== ставки мастеров (для вкладки «Выплаты») ===== */
  const periodLabel = `${year}-${pad2(month)}`;
  const [ratesLoading, setRatesLoading] = useState(false);
  const [ratesError, setRatesError] = useState("");
  // { [barberId]: { id_lesson?, id_month?, perRecord, perMonth } }
  ...
  /* ===== загрузка данных для BarberAnalitika ===== */
  const fetchPaged = async (url) => {
    const acc = [];
    let next = url;
    while (next) {
  ...
  // При смене периода — перезагружаем только ставки
  useEffect(() => {
    setRates({});
    setRatesError("");
    loadRates();
