...
import api from "../../../../api";
import { FaSync, FaUsers, FaCut, FaCalendarAlt, FaListUl, FaTimes } from "react-icons/fa";
import RecordaRates from "../RecordaRates/RecordaRates";

/* ===== ленивые импорты ===== */
const KassaLazy = lazy(() => import("../../../Deposits/Kassa/Kassa"));
const CatalogAnalyticsLazy = lazy(() => import("../CatalogAnalytics/CatalogAnalytics"));
...
const isSupplierLike = (c = {}) => {
  ...
  const text = [
    c.type, c.category, c.group, c.role, c.kind, c.segment, c.status,
...
const CashReports = () => {
  const now = new Date();
  const [tab, setTab] = useState("analytics"); // analytics | catalog | kassa | payouts
  ...
  const loadRates = async () => {
    ...
    try {
      const [lessonRes, monthRes] = await Promise.all([
        api.get(RATES_EP, { params: { period: periodLabel, mode: "lesson", page_size: 1000 } }),
        api.get(RATES_EP, { params: { period: periodLabel, mode: "month", page_size: 1000 } }),
      ]);
    ...
    try {
      ...
      Object.entries(rates).forEach(([barberId, rec]) => {
        const send = async (mode, amount, idKey) => {
          ...
          if (!Number.isFinite(num) || num < 0) return;
          const payload = { teacher: barberId, period: periodLabel, mode, rate: String(num) };
          const id = rec[idKey];
  ...
  useEffect(() => { if (fullView) setModalPage(1); }, [fullView]);
  ...
  const monthsList = [
    { v: 1, t: "Январь" }, { v: 2, t: "Февраль" }, { v: 3, t: "Март" },
    { v: 4, t: "Апрель" }, { v: 5, t: "Май" }, { v: 6, t: "Июнь" },
    { v: 7, t: "Июль" }, { v: 8, t: "Август" }, { v: 9, t: "Сентябрь" },
  ...
  const totalRevenue = useMemo(
    () => filteredApps.reduce((sum, a) => sum + (a.status === "completed" ? priceOf(a) : 0), 0),
    [filteredApps]
  );
  ...
  const abortedRate = totalApps > 0 ? Math.round((abortedCount / totalApps) * 100) : 0;
  ...
  const rankClientsVisits = useMemo(() => {
    ...
    filteredApps.forEach((a) => {
      ...
      const rec = m.get(key) || { id: key, name: clientName(key), count: 0, sum: 0 };
      rec.count += 1;
      rec.sum += priceOf(a);
      m.set(key, rec);
    });
  ...
  const rankServices = useMemo(() => {
    ...
    filteredApps.forEach((a) => {
      ...
      if (!key) return;
      const rec = m.get(key) || { id: key, name: svcName(key), count: 0, sum: 0 };
      if (COUNTABLE_FOR_RANK.has(a.status)) rec.count += 1;
      if (a.status === "completed") rec.sum += priceOf(a);
      m.set(key, rec);
    });
  ...
  const payoutsTotal = useMemo(() => {
    return employees.reduce((sum, e) => {
      const done = filteredApps.filter(
        (a) => a.barber && String(a.barber) === String(e.id) && a.status === "completed"
      ).length;
      const r = rates[e.id] || {};
  ...
  const Pager = ({ page, totalPages, onPrev, onNext }) => {
    ...
    return (
      <nav className="barbercashreports__pager" aria-label="Пагинация">
        <button className="barbercashreports__pageBtn" onClick={onPrev} disabled={page === 1}>Назад</button>
          ...
          {list.map((n, i) => {
            ...
            return (
              <React.Fragment key={n}>
                ...
                <li>
                  <button
                    className={`barbercashreports__pageBtn ${n === page ? "is-active" : ""}`}
        ...
        <button className="barbercashreports__pageBtn" onClick={() => onNext(page + 1)} disabled={page === totalPages}>Далее</button>
      </nav>
    );
  };
  ...
  return (
    <div className="barbercashreports">
      ...
      <div className="barbercashreports__tabs">
        <button className={`barbercashreports__tab ${tab === "analytics" ? "is-active" : ""}`} onClick={() => setTab("analytics")}>
          Аналитика
        </button>
        <button className={`barbercashreports__tab ${tab === "catalog" ? "is-active" : ""}`} onClick={() => setTab("catalog")}>
          Каталог
        </button>
        <button className={`barbercashreports__tab ${tab === "kassa" ? "is-active" : ""}`} onClick={() => setTab("kassa")}>
          Касса
        </button>
        <button className={`barbercashreports__tab ${tab === "payouts" ? "is-active" : ""}`} onClick={() => setTab("payouts")}>
          Выплаты
        </button>
      </div>
        ...
        <>
          <header className="barbercashreports__header">
            ...
            <div className="barbercashreports__actions">
              <div className="barbercashreports__period">
                <select className="barbercashreports__select" value={month}
                  onChange={(e) => { setMonth(Number(e.target.value)); setPageBarbers(1); setPageServices(1); setPageTop10(1); }}
                  aria-label="Месяц" title="Месяц">
                  {monthsList.map((m) => <option key={m.v} value={m.v}>{m.t}</option>)}
                </select>
                <select className="barbercashreports__select" value={year}
                  onChange={(e) => { setYear(Number(e.target.value)); setPageBarbers(1); setPageServices(1); setPageTop10(1); }}
                  aria-label="Год" title="Год">
                  {yearsList.map((y) => <option key={y} value={y}>{y}</option>)}
                </select>
              </div>

              <button className="barbercashreports__btn barbercashreports__btn--primary barbercashreports__btn--icon" onClick={loadAll} title="Обновить данные">
                <FaSync /> <span className="barbercashreports__btnText">Обновить</span>
              </button>
            </div>
          </header>

          {errorMsg && <div className="barbercashreports__alert">{errorMsg}</div>}

          {/* KPI */}
          <section className="barbercashreports__kpi">
            <div className="barbercashreports__card">
              <div className="barbercashreports__cardIcon"><FaCalendarAlt /></div>
              <div className="barbercashreports__cardLabel">Записей (месяц)</div>
              <div className="barbercashreports__cardValue">{fmtInt(totalApps)}</div>
            </div>
            <div className="barbercashreports__card">
              <div className="barbercashreports__cardIcon"><FaCut /></div>
              <div className="barbercashreports__cardLabel">Услуг (всего)</div>
              <div className="barbercashreports__cardValue">{fmtInt(totalServices)}</div>
            </div>
            <div className="barbercashreports__card">
              <div className="barbercashreports__cardIcon"><FaUsers /></div>
              <div className="barbercashreports__cardLabel">Клиенты барбершоп</div>
              <div className="barbercashreports__cardValue">{fmtInt(totalClientsBarber)}</div>
            </div>
            <div className="barbercashreports__card">
              <div className="barbercashreports__cardIcon"><FaUsers /></div>
              <div className="barbercashreports__cardLabel">Клиенты продаж</div>
              <div className="barbercashreports__cardValue">{fmtInt(totalClientsMarket)}</div>
            </div>
            <div className="barbercashreports__card barbercashreports__card--accent">
              ...
              <div className="barbercashreports__cardValue">{fmtMoney(totalRevenue)}</div>
            </div>
            <div className="barbercashreports__card barbercashreports__card--accent">
              <div className="barbercashreports__cardLabel">Расход: выплаты мастерам</div>
              <div className="barbercashreports__cardValue">{fmtMoney(payoutsTotal)}</div>
            </div>
          </section>
          ...
          <section className="barbercashreports__panel">
            ...
            <div className="barbercashreports__statusList">
              {[
                { code: "completed", label: STATUS_LABELS.completed, count: completedCount, sum: completedSum },
                { code: "aborted",  label: "Отмены и не пришёл",     count: abortedCount,  sum: 0 },
              ].map((row) => {
                ...
                return (
                  <div key={row.code} className="barbercashreports__statusRow">
                    <div className="barbercashreports__statusHead">
                      <span className={`barbercashreports__badge barbercashreports__badge--${row.code}`}>{row.label}</span>
                      <span className="barbercashreports__statusNum">{fmtInt(row.count)}</span>
                    </div>
                    <div className="barbercashreports__progress" aria-label={`${row.label} ${share}%`}>
                      <div className="barbercashreports__progressFill" style={{ width: `${share}%` }} />
                    </div>
                    {row.code === "completed" && (
                      <div className="barbercashreports__statusMoney">Сумма: {fmtMoney(row.sum)}</div>
                    )}
                  </div>
                );
              })}
              {!totalApps && <div className="barbercashreports__muted">Нет данных за месяц.</div>}
            </div>
          </section>
          ...
          <section className="barbercashreports__grid">
            ...
            <div className="barbercashreports__panel">
              <div className="barbercashreports__panelHead">
                <h3 className="barbercashreports__panelTitle">Мастера — записи</h3>
                <button className="barbercashreports__iconBtn" title="Показать всё" onClick={() => setFullView("barbers")}><FaListUl /></button>
              </div>
              <div className={`barbercashreports__scroll ${rankBarbers.length <= RANK_PAGE ? "barbercashreports__scroll--center" : ""}`}>
                <ol className="barbercashreports__rankList">
                  {barbersPage.rows.map((r, i) => (
                    <li key={`${r.id}-${i}`} className="barbercashreports__rankItem">
                      <span className="barbercashreports__rankName">{r.name}</span>
                      <span className="barbercashreports__rankCount">{fmtInt(r.count)} • {fmtMoney(r.sum)}</span>
                    </li>
                  ))}
                  {!loading && !rankBarbers.length && <div className="barbercashreports__muted">Нет записей.</div>}
                </ol>
              </div>
              <Pager
                ...
                onNext={(n) => setPageBarbers((p) => (typeof n === "number" ? n : Math.min(barbersPage.totalPages, p + 1)))}
              />
            </div>
            ...
            <div className="barbercashreports__panel">
              <div className="barbercashreports__panelHead">
                <h3 className="barbercashreports__panelTitle">Услуги — использование</h3>
                <button className="barbercashreports__iconBtn" title="Показать всё" onClick={() => setFullView("services")}><FaListUl /></button>
              </div>
              <div className={`barbercashreports__scroll ${rankServices.length <= RANK_PAGE ? "barbercashreports__scroll--center" : ""}`}>
                <ol className="barbercashreports__rankList">
                  ...
                  {!loading && !rankServices.length && <div className="barbercashreports__muted">Нет данных.</div>}
                </ol>
              </div>
              <Pager
                ...
                onNext={(n) => setPageServices((p) => (typeof n === "number" ? n : Math.min(servicesPage.totalPages, p + 1)))}
              />
            </div>
          </section>
          ...
          <section className="barbercashreports__panel">
            <div className="barbercashreports__panelHead">
              ...
              <button className="barbercashreports__iconBtn" title="Показать полный список" onClick={() => setFullView("topClients")}><FaListUl /></button>
            </div>
            ...
            <div className="barbercashreports__topTableWrap">
              <table className="barbercashreports__table">
                ...
                <tbody>
                  ...
                  {!topPage.rows.length && (
                    <tr>
                      <td colSpan="4" className="barbercashreports__muted">Нет данных.</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <Pager
              ...
              onNext={(n) => setPageTop10((p) => (typeof n === "number" ? n : Math.min(topPage.totalPages, p + 1)))}
            />
          </section>

          {/* Модалка полноты */}
          {fullView && (
            <div className="barbercashreports__modalOverlay" onClick={() => setFullView(null)}>
              <div className="barbercashreports__modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
                <div className="barbercashreports__modalHeader">
                  <h3 className="barbercashreports__modalTitle">
                    {fullView === "barbers" ? "Мастера — записи"
                      : fullView === "services" ? "Услуги — использование"
                      : "Клиенты"}
                  </h3>
                  <button className="barbercashreports__iconBtn" onClick={() => setFullView(null)} aria-label="Закрыть"><FaTimes /></button>
                </div>
                  ...
                  {fullView === "topClients" ? (
                    <div className="barbercashreports__topTableWrap">
                      <table className="barbercashreports__table">
                        ...
                        <tbody>
                          {slicePage(rankClientsVisits, modalPage, MODAL_PAGE).rows.map((r) => (
                            <tr key={r.id}>
                              <td>{r.name}</td>
                              <td>{fmtInt(r.count)}</td>
                              <td>{fmtMoney(r.sum)}</td>
                    ...
                    <ol className="barbercashreports__rankList barbercashreports__rankList--full">
                      {slicePage(fullView === "barbers" ? rankBarbers : rankServices, modalPage, MODAL_PAGE).rows.map((r, i) => (
                        <li key={`${r.id}-${i}`} className="barbercashreports__rankItem">
                          <span className="barbercashreports__rankName">{r.name}</span>
                          <span className="barbercashreports__rankCount">{fmtInt(r.count)} • {fmtMoney(r.sum)}</span>
                        </li>
                ...
                <nav className="barbercashreports__pager" aria-label="Пагинация">
                  <button className="barbercashreports__pageBtn" onClick={() => setModalPage((p) => Math.max(1, p - 1))} disabled={modalPage === 1}>Назад</button>
                  <button className="barbercashreports__pageBtn" onClick={() => setModalPage((p) => p + 1)}>Далее</button>
                </nav>

                <div className="barbercashreports__formActions">
                  <button className="barbercashreports__btn barbercashreports__btn--secondary" onClick={() => setFullView(null)}>Закрыть</button>
                </div>
              </div>
            </div>
          )}
      ...
      {tab === "kassa" && (
        <div className="barbercashreports__kassaWrap">
          <Suspense fallback={<div className="barbercashreports__muted">Загрузка кассы…</div>}>
            <KassaLazy />
          </Suspense>
        </div>
      )}
