...
const CashReports = () => {
  ...
  // суммарная сумма выплат по данным endpoint'а payouts
  ...
  /* ==== запись расхода в кассу (как было) ==== */
  const upsertPayoutExpense = async (amountSom) => {
    if (!amountSom || amountSom <= 0) return;
  ...
  const persistRates = async () => {
    ...
    setRatesError("");
    try {
      // 1) сохраняем ставки по новому endpoint'у
      const tasks = [];
      ...
      // 2) перезагружаем payouts и считаем суммарную выплату
      const newRates = await loadRates();
      const payoutTotalSom = calcPayoutTotalForPeriod(newRates);

      // 3) пишем расход в кассу (если нужно)
      await upsertPayoutExpense(payoutTotalSom);
    } catch (e) {
      console.error(e);
      setRatesError("Не удалось сохранить ставки мастеров.");
