...
const toNum = (v) => {
  const n =
    typeof v === "string" ? Number(v.replace(",", ".")) : Number(v ?? 0);
      ...
      <div className="sfcard__body">
        ...
        {originalPrice && originalPrice > price && (
          <div className="sfcard__oldPrice">
            <span className="sfcard__oldPriceValue">
              {money(originalPrice)} сом
            </span>
          </div>
        )}
...
const ShowcaseModal = ({ open, slug, productId, onClose, onAdd }) => {
  ...
  useEffect(() => {
    ...
    api
      .get(
        `/main/public/companies/${encodeURIComponent(slug)}/showcase/${encodeURIComponent(productId)}/`,
      )
      .then((res) => {
        if (!alive) return;
          ...
          {!loading && !err && data && (
            <div className="sfmodal__grid">
              ...
              <div className="sfmodal__right">
                ...
                <div className="sfmodal__priceRow">
                  ...
                  {data?.price &&
                    ...
                    Number(data.price) !== Number(data.final_price) && (
                      <div className="sfmodal__oldPrice">
                        {money(toNum(data.price))} сом
                      </div>
                    )}
                </div>
                ...
                <button
                  type="button"
                  className="sfmodal__cta"
                  onClick={() => onAdd?.(data, 1)}
                >
    ...
    <div className="sfdrawer" role="dialog" aria-modal="true">
      ...
      <div className="sfdrawer__panel">
        ...
        <div className="sfdrawer__foot">
          ...
          {checkoutHint ? (
            <div className="sfdrawer__hint">{checkoutHint}</div>
          ) : null}
          ...
          <button
            ...
            title={
              !canCheckout
                ? "Заполни номер клиента и проверь номер витрины"
                : "Открыть WhatsApp"
            }
          >
            Оформить заказ
...
const Catalog = () => {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((e) => {
        ...
        if (status === 404)
          setCompanyErr("Компания не найдена (404). Проверь slug.");
        else if (status) setCompanyErr(`Ошибка загрузки компании (${status}).`);
        else
          setCompanyErr("Ошибка загрузки компании (проверь baseURL / CORS).");
        setCompany(null);
      })
      .finally(() => {
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((e) => {
        ...
        const status = e?.response?.status;
        if (status === 404)
          setErr("Витрина не найдена (404). Проверь путь API.");
  ...
  const categoriesFromItems = useMemo(() => {
    ...
    (items || []).forEach((p) => {
      ...
      map.set(String(p.category), {
        id: String(p.category),
        title: String(p.category_title),
      });
    });
  ...
  const currentPage = Math.max(
    1,
    Math.min(parseInt(qState.page, 10) || 1, totalPages),
  );
  ...
  const brandTitle =
    company?.name || (companyLoading ? "Загрузка..." : "TechStore");
  ...
  const checkoutHint = useMemo(() => {
    ...
    if (!showcasePhoneRaw)
      return "WhatsApp недоступен: в ответе API компании (GET /main/public/companies/{slug}/) нет поля phones_howcase. Добавьте его в сериализатор публичной компании и укажите номер в настройках «Моя компания».";
    if (!phoneShowcaseDigits)
      return "WhatsApp недоступен: неверный номер витрины.";
        ...
        <div className="sfbar__inner">
          ...
          <div className="sfbar__search">
            ...
            {qState.q ? (
              <button
                type="button"
                className="sfbar__clear"
                onClick={() => setQS({ q: "", page: "" })}
                aria-label="Clear"
              >
                <FaTimes />
              </button>
          ...
          <div className="sfbar__right">
            ...
            >
              ...
              <span>Корзина</span>
              {cartCount > 0 ? (
                <span className="sfbar__cartBadge">{cartCount}</span>
              ) : null}
            </button>
          </div>
        </div>
        ...
        {company && !companyLoading && (
          <div className="shopfront__hero">
            <div className="shopfront__heroContent">
              <h1 className="shopfront__heroTitle">
                {company.name || "Добро пожаловать"}
              </h1>
              {company.description && (
