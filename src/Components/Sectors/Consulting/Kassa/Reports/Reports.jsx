import React, { useEffect, useMemo, useState } from "react";
import styles from "./Reports.module.scss";
import { FaChartLine, FaPrint, FaSync, FaFilter } from "react-icons/fa";
import api from "../../../../../api";

/* ────────── утилиты ────────── */
const listFrom = (res) => res?.data?.results || res?.data || [];
const toNum = (v) => {
  if (v == null) return 0;
  const n = Number(String(v).replace(",", "."));
  return Number.isFinite(n) ? n : 0;
};
const fmt = (n) => new Intl.NumberFormat("ru-RU").format(Number(n) || 0);
const fmtMoney = (n) =>
  new Intl.NumberFormat("ru-RU", {
    minimumFractionDigits: 0,
    maximumFractionDigits: 0,
  }).format(Math.round(Number(n) || 0)) + " сом";

const RU_MONTHS = [
  "Янв",
  "Фев",
  "Мар",
  "Апр",
  "Май",
  "Июн",
  "Июл",
  "Авг",
  "Сен",
  "Окт",
  "Ноя",
  "Дек",
];
const ymKey = (d) => {
  const dt = new Date(d);
  if (Number.isNaN(dt.getTime())) return null;
  const y = dt.getFullYear();
  const m = String(dt.getMonth() + 1).padStart(2, "0");
  return `${y}-${m}`;
};
const ymLabel = (ym) => {
  const [y, m] = ym.split("-").map(Number);
  return `${RU_MONTHS[(m || 1) - 1]} ${y}`;
};
const inRange = (iso, fromStr, toStr) => {
  if (!fromStr && !toStr) return true;
  if (!iso) return false;
  const t = new Date(iso).getTime();
  if (Number.isNaN(t)) return false;
  const from = fromStr ? new Date(`${fromStr}T00:00:00`).getTime() : -Infinity;
  const to = toStr ? new Date(`${toStr}T23:59:59.999`).getTime() : +Infinity;
  return t >= from && t <= to;
};
const curYear = new Date().getFullYear();

/* ────────── компонент ────────── */
export default function ConsultingReports() {
  const [period, setPeriod] = useState("ytd"); // ytd | 6m | 12m
  const [loading, setLoading] = useState(false);

  // кассы (из консалтинга, если есть)
  const [boxes, setBoxes] = useState([]);
  const [boxId, setBoxId] = useState("ALL");

  // движения
  const [flows, setFlows] = useState([]);

  // фильтры
  const [queryMonth, setQueryMonth] = useState("");
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");

  /* ── кассы из консалтинга (если нет — работаем без селекта) */
  const fetchBoxes = async () => {
    try {
      const r = await api
        .get("/consulting/cashboxes/")
        .catch(() => ({ data: [] }));
      const arr = listFrom(r) || [];
      setBoxes(arr);
      setBoxId("ALL");
    } catch {
      setBoxes([]);
      setBoxId("ALL");
    }
  };

  /* ── загрузка всех движений из консалтинга (первый успешный источник) */
  const fetchAllFlows = async () => {
    setLoading(true);
    try {
      // параметр кассы (если выбран)
      const boxParam =
        boxId && boxId !== "ALL" ? `cashbox=${encodeURIComponent(boxId)}` : "";

      // универсальная пагинация DRF / или массив без пагинации
      const fetchPaged = async (baseUrl) => {
        let url = boxParam
          ? `${baseUrl}${baseUrl.includes("?") ? "&" : "?"}${boxParam}`
          : baseUrl;
        const acc = [];
        let guard = 0;
        while (url && guard < 60) {
          const { data } = await api.get(url);
          if (Array.isArray(data)) {
            acc.push(...data);
            url = null;
          } else {
            const chunk = data?.results || [];
            if (Array.isArray(chunk)) acc.push(...chunk);
            url = data?.next || null;
          }
          guard += 1;
        }
        return acc;
      };

      // 1) пробуем /consulting/cashflows/
      let acc = [];
      try {
        acc = await fetchPaged("/consulting/cashflows/");
      } catch {}

      // 2) иначе /consulting/transactions/
      if (!acc.length) {
        try {
          acc = await fetchPaged("/consulting/transactions/");
        } catch {}
      }

      // 3) иначе payments (income) + expenses (expense)
      if (!acc.length) {
        try {
          const inc = await fetchPaged("/consulting/payments/");
          const exp = await fetchPaged("/consulting/expenses/");
          // помечаем тип вручную
          inc.forEach((x) => (x.__forcedType = "income"));
          exp.forEach((x) => (x.__forcedType = "expense"));
          acc = [...inc, ...exp];
        } catch {}
      }

      // если всё пусто — финальная попытка на общий fallback
      if (!acc.length) {
        try {
          // вдруг на бэке лежит в другом названии
          acc = await fetchPaged("/consulting/finance/flows/");
        } catch {}
      }

      // маппинг в унифицированный формат
      const mapped = (acc || []).map((x, i) => {
        const rawAmt = toNum(x.amount ?? x.sum ?? x.value ?? x.total);
        let type = (
          x.__forcedType ||
          String(x.type ?? x.kind ?? x.direction ?? "").toLowerCase()
        ).trim();
        if (type !== "income" && type !== "expense") {
          type = rawAmt < 0 ? "expense" : "income";
        }
        return {
          id: x.id || x.uuid || `${i}`,
          type, // income | expense
          amount: Math.abs(rawAmt),
          created_at:
            x.created_at ||
            x.created ||
            x.date ||
            x.timestamp ||
            x.createdAt ||
            null,
          title:
            x.title ||
            x.name ||
            x.description ||
            x.note ||
            (type === "income" ? "Приход" : "Расход"),
          cashbox: x.cashbox || x.cashbox_id || null,
        };
      });

      setFlows(mapped);
    } catch (e) {
      console.error("Ошибка загрузки consulting flows:", e);
      setFlows([]);
    } finally {
      setLoading(false);
    }
  };

  // первая загрузка
  useEffect(() => {
    (async () => {
      await fetchBoxes();
    })();
  }, []);

  // перезагрузка движений при смене кассы
  useEffect(() => {
    fetchAllFlows();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [boxId]);

  // фильтрация по датам
  const flowsFiltered = useMemo(() => {
    if (!dateFrom && !dateTo) return flows;
    return flows.filter((f) => inRange(f.created_at, dateFrom, dateTo));
  }, [flows, dateFrom, dateTo]);

  // группировка по месяцам
  const grouped = useMemo(() => {
    const map = new Map();
    for (const f of flowsFiltered) {
      const ym = ymKey(f.created_at);
      if (!ym) continue;
      const cur = map.get(ym) || { income: 0, expense: 0, ops: 0 };
      if (f.type === "income") cur.income += toNum(f.amount);
      else cur.expense += toNum(f.amount);
      cur.ops += 1;
      map.set(ym, cur);
    }
    return Array.from(map.entries())
      .map(([ym, v]) => ({
        ym,
        label: ymLabel(ym),
        income: v.income,
        expense: v.expense,
        net: v.income - v.expense,
        ops: v.ops,
      }))
      .sort((a, b) => (a.ym < b.ym ? -1 : 1));
  }, [flowsFiltered]);

  // период
  const data = useMemo(() => {
    if (!grouped.length) return [];
    if (dateFrom || dateTo) return grouped;
    if (period === "6m") return grouped.slice(-6);
    if (period === "12m") return grouped.slice(-12);
    if (period === "ytd")
      return grouped.filter((r) => Number(r.ym.slice(0, 4)) === curYear);
    return grouped;
  }, [grouped, period, dateFrom, dateTo]);

  // сводка
  const summary = useMemo(() => {
    const income = data.reduce((s, d) => s + d.income, 0);
    const expense = data.reduce((s, d) => s + d.expense, 0);
    const net = income - expense;
    return { income, expense, net };
  }, [data]);

  // данные графика
  const seriesIncome = useMemo(
    () => data.map((d) => ({ label: d.label, value: d.income })),
    [data]
  );
  const seriesExpense = useMemo(
    () => data.map((d) => ({ label: d.label, value: d.expense })),
    [data]
  );
  const maxY = useMemo(
    () =>
      Math.max(
        1,
        ...seriesIncome.map((s) => s.value),
        ...seriesExpense.map((s) => s.value)
      ),
    [seriesIncome, seriesExpense]
  );

  const makePolyline = (series) => {
    const W = 640,
      H = 220,
      P = 24;
    const innerW = W - P * 2;
    const innerH = H - P * 2;
    const points = series.map((s, i) => {
      const x =
        P + innerW * (series.length === 1 ? 0 : i / (series.length - 1));
      const y = P + (innerH - (s.value / maxY) * innerH);
      return `${x},${y}`;
    });
    return { points: points.join(" ") };
  };

  const polyIncome = useMemo(
    () => makePolyline(seriesIncome),
    [seriesIncome, maxY]
  );
  const polyExpense = useMemo(
    () => makePolyline(seriesExpense),
    [seriesExpense, maxY]
  );

  // поиск по названию месяца
  const tableData = useMemo(() => {
    const q = (queryMonth || "").trim().toLowerCase();
    if (!q) return data;
    return data.filter((d) => d.label.toLowerCase().includes(q));
  }, [data, queryMonth]);

  const handleRefresh = () => fetchAllFlows();

  return (
    <section className={styles.reports}>
      {/* Header */}
      <div className={styles.reports__header}>
        <div>
          <h2 className={styles.reports__title}>
            <FaChartLine /> Отчёты и аналитика (консалтинг)
          </h2>
        </div>

        <div className={styles.reports__actions}>
          {/* выбор кассы, если есть */}
          {boxes.length > 0 ? (
            <select
              className={styles.reports__select}
              value={boxId}
              onChange={(e) => setBoxId(e.target.value)}
              title="Касса"
            >
              <option value="ALL">Все кассы</option>
              {boxes.map((b) => (
                <option key={b.id || b.uuid} value={b.id || b.uuid}>
                  {b.department_name || b.name || "Касса"}
                </option>
              ))}
            </select>
          ) : (
            <span className={styles.reports__muted}>Кассы: все</span>
          )}

          <button
            className={`${styles.reports__btn} ${styles["reports__btn--secondary"]}`}
            onClick={() => window.print()}
          >
            <FaPrint /> Печать
          </button>
          <button
            className={`${styles.reports__btn} ${styles["reports__btn--secondary"]}`}
            onClick={handleRefresh}
            disabled={loading}
            title="Обновить данные"
          >
            <FaSync /> Обновить
          </button>
        </div>
      </div>

      {/* Controls */}
      <div className={styles.reports__controls}>
        <div className={styles.reports__filter}>
          <FaFilter className={styles.reports__filterIcon} />
          <select
            className={styles.reports__select}
            value={period}
            onChange={(e) => setPeriod(e.target.value)}
            disabled={!!(dateFrom || dateTo)}
            title={
              dateFrom || dateTo
                ? "Период отключён: используется диапазон дат"
                : "Период"
            }
          >
            <option value="6m">Последние 6 мес</option>
            <option value="ytd">С начала года</option>
            <option value="12m">12 месяцев</option>
          </select>
        </div>

        {/* Дата от / до */}
        <input
          type="date"
          className={styles.reports__select}
          value={dateFrom}
          onChange={(e) => setDateFrom(e.target.value)}
          title="Дата от"
        />
        <input
          type="date"
          className={styles.reports__select}
          value={dateTo}
          onChange={(e) => setDateTo(e.target.value)}
          title="Дата до"
        />

        <div className={styles.reports__series}>
          <label className={styles.reports__check}>
            <input type="checkbox" checked readOnly />
            <span
              className={`${styles.reports__dot} ${styles["reports__dot--income"]}`}
            />
            Приход
          </label>
          <label className={styles.reports__check}>
            <input type="checkbox" checked readOnly />
            <span
              className={`${styles.reports__dot} ${styles["reports__dot--expense"]}`}
            />
            Расход
          </label>

          <input
            className={styles.reports__select}
            placeholder="Фильтр месяца (напр. Июл)"
            value={queryMonth}
            onChange={(e) => setQueryMonth(e.target.value)}
            style={{ minWidth: 160 }}
            title="Поиск по названию месяца"
          />
        </div>
      </div>

      {/* KPIs */}
      <div className={styles.reports__kpis}>
        <div className={styles.reports__kpi}>
          <div className={styles.reports__kpiLabel}>Приход</div>
          <div className={styles.reports__kpiValue}>
            {fmtMoney(summary.income)}
          </div>
        </div>
        <div className={styles.reports__kpi}>
          <div className={styles.reports__kpiLabel}>Расход</div>
          <div className={styles.reports__kpiValue}>
            {fmtMoney(summary.expense)}
          </div>
        </div>
        <div className={styles.reports__kpi}>
          <div className={styles.reports__kpiLabel}>Итог</div>
          <div className={styles.reports__kpiValue}>
            {fmtMoney(summary.net)}
          </div>
        </div>
      </div>

      {/* Chart */}
      <div className={styles.reports__chartCard}>
        <div className={styles.reports__chartHead}>
          Динамика: приход vs расход
        </div>

        <div className={styles.reports__chart}>
          <div
            className={styles.reports__chartLabels}
            style={{ "--cols": data.length || 1 }}
          >
            {data.map((d) => (
              <span key={d.ym} className={styles.reports__chartLabel}>
                {d.label}
              </span>
            ))}
          </div>

          <svg
            className={styles.reports__svg}
            viewBox="0 0 640 220"
            preserveAspectRatio="none"
            aria-hidden
          >
            {[0.25, 0.5, 0.75, 1].map((g) => (
              <line
                key={g}
                x1="24"
                x2="616"
                y1={24 + (220 - 48) * g}
                y2={24 + (220 - 48) * g}
                className={styles.reports__grid}
              />
            ))}

            {/* Приход */}
            <polyline
              className={`${styles.reports__poly} ${styles["reports__poly--income"]}`}
              points={polyIncome.points}
            />
            {polyIncome.points
              .split(" ")
              .filter(Boolean)
              .map((pt, i) => {
                const [cx, cy] = pt.split(",");
                return (
                  <circle
                    key={`inc-${i}`}
                    cx={cx}
                    cy={cy}
                    r="3"
                    className={`${styles.reports__dotPoint} ${styles["reports__dotPoint--income"]}`}
                  />
                );
              })}

            {/* Расход */}
            <polyline
              className={`${styles.reports__poly} ${styles["reports__poly--expense"]}`}
              points={polyExpense.points}
            />
            {polyExpense.points
              .split(" ")
              .filter(Boolean)
              .map((pt, i) => {
                const [cx, cy] = pt.split(",");
                return (
                  <circle
                    key={`exp-${i}`}
                    cx={cx}
                    cy={cy}
                    r="3"
                    className={`${styles.reports__dotPoint} ${styles["reports__dotPoint--expense"]}`}
                  />
                );
              })}
          </svg>
        </div>

        <div className={styles.reports__legend}>
          <span>
            <i
              className={`${styles.reports__legendDot} ${styles["reports__legendDot--income"]}`}
            />
            Приход
          </span>
          <span>
            <i
              className={`${styles.reports__legendDot} ${styles["reports__legendDot--expense"]}`}
            />
            Расход
          </span>
        </div>
      </div>

      {/* Table */}
      <div className={styles.reports__tableWrap}>
        <table className={styles.reports__table}>
          <thead>
            <tr>
              <th>Месяц</th>
              <th>Приход</th>
              <th>Расход</th>
              <th>Итог</th>
              <th>Операций</th>
            </tr>
          </thead>
          <tbody>
            {tableData.map((d) => (
              <tr key={d.ym}>
                <td>{d.label}</td>
                <td>{fmtMoney(d.income)}</td>
                <td>{fmtMoney(d.expense)}</td>
                <td>{fmtMoney(d.net)}</td>
                <td>{fmt(d.ops)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr>
              <td>Итого</td>
              <td>{fmtMoney(summary.income)}</td>
              <td>{fmtMoney(summary.expense)}</td>
              <td>{fmtMoney(summary.net)}</td>
              <td>{fmt(tableData.reduce((s, d) => s + d.ops, 0))}</td>
            </tr>
          </tfoot>
        </table>
      </div>

      {loading && <div className={styles.reports__alert}>Загрузка данных…</div>}
      {!loading && !data.length && (
        <div className={styles.reports__alert}>
          Нет данных консалтинга за выбранный период.
        </div>
      )}
    </section>
  );
}
