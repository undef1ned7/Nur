...
const GlobalAlertModal = ({ isError, message, isOpen, onConfirm }) => {
  ...
  return (
    <ReactPortal wrapperId="modal">
      <div className="fixed top-0 left-0 flex w-full h-full justify-center items-center z-10001">
          ...
          <div className="max-h-75 overflow-y-auto">
            <div className="mx-auto flex mt-2.5 p-2.25 w-13.75 h-13.75  justify-center items-center rounded-full bg-[#e4e6e7]">
              {isError ? (
                ...
                <NotificationsIcon color="info" className="w-full h-full" />
              )}
            </div>
            <h3 className="text-[20px] font-bold text-[#575b61] text-center my-2">
              {isError ? "Ошибка" : "Оповощение"}
            </h3>
