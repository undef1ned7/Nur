...
const Kassa = () => {
  ...
  // const filteredCashboxes = cashboxes.filter(
  //   (cashbox) =>
  //     ((cashbox.name &&
  //       cashbox.name.toLowerCase().includes(searchTerm.toLowerCase())) ||
  //       (cashbox.id &&
  //         cashbox.id.toLowerCase().includes(searchTerm.toLowerCase()))) &&
  //     (!typeMap[activeTab] || cashbox.type === typeMap[activeTab])
  // );
  const isFalse = (v) => v === false || String(v).toLowerCase() === "false";

  const filteredCashboxes = cashboxes.filter((cashbox) => {
    if (isFalse(cashbox?.status)) return false;
  ...
  const handleEditCashbox = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://app.nurcrm.kg/api/construction/cashflows/${selectedCashbox.id}/`,
        {
          method: "POST", // Or 'PUT'
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="vitrina">
      <div className="vitrina__header">
        <div className="vitrina__tabs">
          ...
          <span
            className={`vitrina__tab ${
              activeTab === "Расход" ? "vitrina__tab--active" : ""
            }`}
          ...
          <span
            className={`vitrina__tab ${
              activeTab === "Приход" ? "vitrina__tab--active" : ""
            }`}
        ...
        <div className="vitrina__actions">
          <div class="vitrina__controls">
            <div class="vitrina__search-wrapper">
              <svg
                ...
                class="lucide lucide-search vitrina__search-icon"
              >
                <circle cx="11" cy="11" r="8"></circle>
                <path d="m21 21-4.3-4.3"></path>
              </svg>
              <input class="vitrina__search" type="text" placeholder="Поиск" />
            </div>
            <button class="vitrina__filter-button">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="18"
                height="18"
                ...
                {filteredCashboxes.length > 0 ? (
                  filteredCashboxes.map((cashbox, index) => (
                    <tr key={cashbox.id}>
                      ...
                      <td>
                        {new Date(cashbox.created_at).toLocaleDateString()}
                      </td>
                      <td className="vitrina__actions">
                        {/* <MoreVertical
                                    ...
                                    /> */}
                        <button
                          className="vitrina__button vitrina__button--reset"
                          onClick={() => {
                            setSelectedCashbox(cashbox);
                            handleDeleteCashbox();
                          }}
      ...
      <div className="vitrina__pagination">
        <span className="vitrina__pagination-info">
          1 из {Math.ceil(filteredCashboxes.length / 10) || 1}
        </span>
          ...
          <div className="vitrina__modal vitrina__modal--add">
            ...
            <div className="vitrina__modal-section">
              ...
              <input
                ...
                onChange={(e) =>
                  setNewCashbox({ ...newCashbox, amount: e.target.value })
                }
              />
            </div>
