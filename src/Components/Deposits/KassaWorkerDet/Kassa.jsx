...
const KassaDet = () => {
  ...
  const [activeFlowType, setActiveFlowType] = useState("all"); // 'all', 'income', 'expense', "pending"
  ...
  const filteredCashflows = (cashboxDetails?.cashflows || []).filter((flow) => {
    ...
    const statusOk = flow.status === true; // показываем только подтверждённые операции
