...
export default function BuildingClients() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormError(
        validateResErrors(err, "Не удалось сохранить клиента")
      );
    }
  };
  ...
  const handleDelete = (client) => {
    ...
    if (!id) return;
    confirm(
      `Удалить клиента «${client?.name || "без имени"}»?`,
      async (ok) => {
        if (!ok) return;
        try {
          ...
          alert(
            validateResErrors(err, "Не удалось удалить клиента"),
            true
          );
        }
      }
    ...
    <div className="building-page building-page--clients">
      ...
      <div className="building-page__card">
        <div className="building-page__filters">
          ...
          />
          <label className="building-page__muted" style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <input
              type="checkbox"
              checked={onlyActive}
      ...
      <div className="building-page__card">
                    ...
                    <tr key={id}>
                      ...
                      <td>
                        {c?.is_active ? (
                          ...
                          <span className="building-page__status is-danger">Отключён</span>
                        )}
                      </td>
                      <td>
                        <BuildingActionsMenu
        ...
        {(createError || updatingError) && (
          <div className="building-page__error" style={{ marginTop: 8 }}>
            {String(
              validateResErrors(createError || updatingError, "Ошибка при сохранении клиента")
            )}
          </div>
        )}
      </div>
