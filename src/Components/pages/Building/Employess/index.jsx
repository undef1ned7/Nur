// Сотрудники — раздел Строительство (скопировано из Barber/Masters)
...
const Masters = () => {
  ...
  const { items: residentialComplexes } = useBuildingProjects();
  ...
  const [accessModalResidentialIds, setAccessModalResidentialIds] = useState([]);
  const [accessModalResidentialIdsInitial, setAccessModalResidentialIdsInitial] = useState([]);
  ...
  const openAccessModal = async (employee) => {
    try {
      ...
      setAccessModalResidentialIds([]);
      setAccessModalResidentialIdsInitial([]);
  ...
  useEffect(() => {
    if (!accessModalOpen || !accessModalEmployee?.id || !residentialComplexes?.length) return;
    const list = Array.isArray(residentialComplexes) ? residentialComplexes : [];
    const userId = accessModalEmployee.id;
    let cancelled = false;
    (async () => {
      const ids = [];
      for (const rc of list) {
        ...
        if (!rcId) continue;
        try {
          const { data } = await api.get(`/building/objects/${rcId}/members/`);
          const members = Array.isArray(data) ? data : [];
          if (members.some((m) => String(m?.user ?? m?.user_id) === String(userId)))
            ids.push(rcId);
        } catch {
          // ignore
        }
      ...
      if (!cancelled) {
        setAccessModalResidentialIdsInitial(ids);
        setAccessModalResidentialIds(ids);
      }
    })();
    return () => { cancelled = true; };
  }, [accessModalOpen, accessModalEmployee?.id, residentialComplexes]);
  ...
  const handleSaveEmployeeAccesses = async (newAccessesPayload) => {
    ...
    setPageNotice("");
    try {
      await api.patch(EMPLOYEE_ITEM_URL(userId), newAccessesPayload);
      const currentIds = Array.isArray(accessModalResidentialIds) ? accessModalResidentialIds : [];
      const prevIds = Array.isArray(accessModalResidentialIdsInitial) ? accessModalResidentialIdsInitial : [];
      const toAdd = currentIds.filter((id) => !prevIds.includes(id));
      const toRemove = prevIds.filter((id) => !currentIds.includes(id));
      for (const rcId of toAdd) {
        if (!rcId) continue;
        try {
          await api.post(`/building/objects/${rcId}/members/`, {
            user: String(userId),
            is_active: true,
          });
        } catch (errRc) {
          console.error("Не удалось добавить сотрудника в ЖК:", rcId, errRc);
        }
      }
      for (const rcId of toRemove) {
        if (!rcId) continue;
        try {
          await api.delete(`/building/objects/${rcId}/members/${userId}/`);
        } catch (errRc) {
          console.error("Не удалось убрать сотрудника из ЖК:", rcId, errRc);
        }
      }
      await fetchEmployees();
  ...
  return (
    <div className="barbermasters">
      ...
      <EmployeeAccessModal
        ...
        residentialComplexIds={accessModalResidentialIds}
        onResidentialComplexIdsChange={setAccessModalResidentialIds}
      />
