...
export const fetchSalesHistoryApi = async (params = {}) => {
  try {
    // Загружаем все данные без параметров, так как сервер не поддерживает фильтрацию и пагинацию
    const response = await api.get("/main/pos/sales/");
    return response.data;
  } catch (error) {
    if (error.response) {
      console.error("Fetch Sales History Error Data:", error.response.data);
      console.error("Fetch Sales History Error Status:", error.response.status);
      return Promise.reject(error.response.data);
    }
  }
};
