...
function isChatMessageEvent(type) {
  const t = String(type || "").toLowerCase();
  if (!t) return false;
  if (CHAT_MESSAGE_EVENTS.has(t)) return true;
  return (
    (t.includes("message") || t.includes("chat") || t.includes("wazzup")) &&
    !t.includes("lead")
  );
}
...
export function useFunnelBoardWebSocket({
  ...
  // Keep callback refs fresh without writing during render (eslint react-hooks/refs).
  useEffect(() => {
  ...
  }, [onUpsert, onRemove, onAssigned, onReconnect, onChatMessage]);
